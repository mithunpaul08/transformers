...
if [ $MACHINE_TO_RUN_ON == "hpc" ]; then
        ...
        export OUTPUT_DIR_BASE="/home/u11/mithunpaul/xdisk/huggingface_bert_fnc_to_fever_delex_from_stand_alone_code/output"
        export DATA_DIR_BASE="/home/u11/mithunpaul/xdisk/huggingface_bert_fnc_to_fever_delex_from_stand_alone_code/data"
else
        wandb off
...
export TASK_TYPE="delex" #options for task type include mod1,mod2,and combined"". combined is used in case of student teacher architecture which will load a paralleldataset from both mod1 and mod2 folders
...
--task_type $TASK_TYPE --subtask_type $SUB_TASK_TYPE --machine_to_run_on $MACHINE_TO_RUN_ON --toy_data_dir_path $TOY_DATA_DIR_PATH --overwrite_cache"
